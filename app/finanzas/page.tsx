...
import { formatTransactionDate, getTransactionCategoryMeta } from '@/src/modules/finanzas/transaccion.actions'
...
import type { Transaction, TransactionTab } from '@/src/shared/types/finance'

function Transacciones() {
  const { transactions, deleteTransaction } = useAppStore()
  const router = useRouter()
  const [activeTab, setActiveTab] = useState<TransactionTab>('all')
  const [openMenuId, setOpenMenuId] = useState<Transaction['id'] | null>(null)
  ...
  const openDeleteModal = (transaction: Transaction): void => {
  ...
  const toggleTransactionMenu = (transactionId: Transaction['id']): void => {
    setOpenMenuId((current) => (current === transactionId ? null : transactionId))
  }

  const openEditPage = (transactionId: Transaction['id']): void => {
    setOpenMenuId(null)
            ...
            <table className="table table-hover align-middle mb-0">
              ...
              <tbody>
                {filteredTransactions.map((tx) => {
                  const meta = getTransactionCategoryMeta(tx)

                  return (
                    <tr key={tx.id}>
                      <td className="small text-secondary">{formatTransactionDate(tx.date)}</td>
                      <td>
                ...
                <div className="modal-body">
                  ...
                  <div className="rounded border bg-light p-3">
                    ...
                    <p className="mb-0 small text-secondary">
                      {formatTransactionDate(selectedTransaction?.date)} • {selectedTransaction?.accountName || 'Sin cuenta'} • {selectedTransaction?.amount && selectedTransaction.amount >= 0 ? '+' : '-'}${Math.abs(selectedTransaction?.amount || 0).toFixed(2)}
                    </p>
                  </div>
                </div>
