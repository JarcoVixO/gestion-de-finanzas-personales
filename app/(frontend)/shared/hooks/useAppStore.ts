...
const getNextId = <T extends { id: string | number }>(items: T[], prefix: string): string => {
...
const getInitialState = (): PersistedAppStoreState => ({
  count: 0,
  accounts: [
...
const useAppStore = create<AppStoreState>()(
  persist(
    (set) => ({
      ...
      addAccount: ({ name, balance, type, goal }: AddAccountInput) => set((state) => {
        ...
        const numericGoal = safeGoal && safeGoal > 0
          ? safeGoal
          : Math.max(Math.round(Math.abs(numericBalance) * 1.2), 100)

        const newAccount: Account = {
          id: getNextId(state.accounts, 'acc'),
          name: normalizedName,
      ...
      updateAccount: ({ id, name, balance, goal }: UpdateAccountInput) => set((state) => {
        const normalizedName = (name || '').trim()
        const numericBalance = Number.isFinite(balance) ? balance : 0
        const numericGoal = typeof goal === 'number' && Number.isFinite(goal) && goal > 0 ? goal : null
      ...
      addBudget: ({ name, limit, spent, description, category, priority, dueDate }: AddBudgetInput) => set((state) => {
        const normalizedName = (name || '').trim()
        const numericLimit = Number.isFinite(limit) ? limit : 0
        const numericSpent = typeof spent === 'number' && Number.isFinite(spent) ? spent : 0
      ...
      updateBudget: ({ id, name, limit, description, category, priority, dueDate }: UpdateBudgetInput) => set((state) => {
        const normalizedName = (name || '').trim()
      ...
      addBudgetExpense: ({ id, amount, accountId }: AddBudgetExpenseInput) => set((state) => {
      ...
      addTransaction: (tx: AddTransactionInput) => set((state) => {
      ...
      updateTransaction: ({ id, description, amount, date, accountId }: UpdateTransactionInput) => set((state) => ({
        transactions: state.transactions.map((transaction) => {
          ...
          return {
            ...
            amount: typeof amount === 'number' && Number.isFinite(amount) ? amount : transaction.amount,
      ...
      deleteTransaction: (id: string | number) => set((state) => ({
        transactions: state.transactions.filter((transaction) => transaction.id !== id)
      }))
    }),
    {
      name: STORE_KEY,
      storage: createJSONStorage<PersistedAppStoreState>(() => localStorage),
...
export { useAppStore, useAppStore as useStore }
